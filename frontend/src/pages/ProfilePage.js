  ...
  return (
    <div className={`profile-theme-shell min-h-screen pb-8 text-white ${isTitaniumTheme ? 'profile-theme-titanium bg-[#17150f]' : 'profile-theme-obsidian bg-[#080b12]'}`}>
      ...
      <div className={`profile-theme-header relative overflow-hidden border-b border-white/10 ${isTitaniumTheme ? 'bg-[#211d13]' : 'bg-[#0b1220]'}`}>
        ...
        <div className="relative mx-auto max-w-6xl px-4 pb-20 pt-6 sm:px-6 sm:pb-28 sm:pt-7 lg:px-8">
      ...
      <main className="relative mx-auto -mt-12 max-w-6xl px-4 sm:-mt-20 sm:px-6 lg:px-8">
        ...
        {profileUser.user_kind === 'professional' && (
          <section className="executive-tile mt-4 rounded-[26px] border border-white/10 bg-[#101621] p-5 sm:p-6"><div className="flex items-start gap-3"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-[#2d82ff]/15 text-[#8db7ff]"><Briefcase size={20} /></div><div className="min-w-0"><p className="text-[10px] font-bold uppercase tracking-[.25em] text-[#7eaeff]">Professional showcase</p><h2 className="mt-1 text-lg font-bold leading-tight sm:text-xl">Your edge, clearly presented</h2></div></div><div className="mt-5 flex flex-wrap gap-2">{(profileUser.skills || []).length ? profileUser.skills.map((skill) => <span key={skill} className="rounded-full border border-white/10 bg-white/[.04] px-3 py-2 text-xs text-white/65">{skill}</span>) : <p className="text-sm italic text-white/40">No skills yet — tap Edit profile to add some.</p>}</div><button onClick={() => navigate('/jobs')} className="mt-5 inline-flex items-center gap-2 rounded-xl bg-[#2d82ff] px-4 py-3 text-sm font-bold text-white transition hover:bg-[#438fff] active:scale-[.97]">Browse opportunities <ArrowUpRight size={16} /></button></section>
        )}

        <section className="executive-tile mt-4 rounded-[26px] border border-white/10 bg-[#101621] p-5 sm:p-6"><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[.25em] text-white/35">Profile media</p><h2 className="mt-1 text-xl font-bold">Proof of work</h2></div><div className="scrollbar-hide flex max-w-full min-w-0 gap-1 overflow-x-auto rounded-full bg-white/[.05] p-1">{[['photos', Camera, photos.length], ['videos', Video, videos.length], ['articles', FileText, articles.length]].map(([tab, Icon, count]) => <button key={tab} onClick={() => setMediaTab(tab)} className={`inline-flex shrink-0 items-center gap-1.5 whitespace-nowrap rounded-full px-3 py-2 text-xs font-bold transition ${mediaTab === tab ? 'bg-[#2d82ff] text-white' : 'text-white/45 hover:text-white'}`}><Icon size={13} />{tab[0].toUpperCase() + tab.slice(1)} <span className="opacity-60">{count}</span></button>)}</div></div>
