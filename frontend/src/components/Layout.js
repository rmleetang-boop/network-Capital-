...
const Layout = ({ children, user, onLogout }) => {
  ...
  return (
    <div className={`min-h-screen w-full overflow-x-hidden bg-background-DEFAULT ${hideBottomNav ? '' : 'pb-20'}`}>
      <div className="w-full min-w-0">{children}</div>

      {!hideBottomNav && (
        <nav className="fixed bottom-0 left-0 right-0 w-full bg-white/80 backdrop-blur-lg border-t border-gray-200 z-40">
